...
# 进程内内存缓存，结构: { md5_key: {"value": str, "expires_at": float} }
...
def set_cache(question: str, answer: str) -> None:
    """
    将问答结果写入缓存，TTL 为 1 小时。
    """
    key = _make_key(question)
    _store[key] = {
        "value":      answer,
        "expires_at": time.time() + TTL_SECONDS,    # 绝对过期时间点（非滑动窗口）
    }
    logger.debug(f"[Cache] Set: {key}")


def get_cache(question: str) -> str | None:
    """
    查询缓存，命中且未过期则返回答案，否则返回 None。
    """
    key = _make_key(question)
